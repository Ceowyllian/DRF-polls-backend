...
class TestQuestionList:
    ...
    def test_filter_by_title(self, user):
        expected_title = "aaaa"
        ...
        for _ in range(3):
            expected_questions.add(
                Question.objects.create(
                    title=expected_title, text="blablabla", created_by=user
                )
            )
        ...
        for _ in range(3):
            unsuitable_questions.add(
                Question.objects.create(title="bbbb", text="blablabla", created_by=user)
            )

        results = services.question.question_list(filters={"title": expected_title})
    ...
    def test_filter_by_text(self, user):
        expected_text = "aaaa"
        ...
        for _ in range(3):
            expected_questions.add(
                Question.objects.create(
                    title="blablabla", text=expected_text, created_by=user
                )
            )
        ...
        for _ in range(3):
            unsuitable_questions.add(
                Question.objects.create(title="blablabla", text="bbbb", created_by=user)
            )

        results = services.question.question_list(filters={"text": expected_text})
